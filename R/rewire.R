...
rewire_directed <- function(edgelist, eta, 
  ...
  result <- list(rho = rho, rankRho = rankRho,
                 edgelist = edgelist)
  if (rewire_track) {
    colnames(ret$rewire_history) <- c("Attempt", "Edge1", "Edge2", "Accepted")
    ret$rewire_history[, 1:3] <- ret$rewire_history[, 1:3] + 1
    result$rewire_history <- ret$rewire_history
  }
  return(result)
}
...
#' ret1 <- joint_dist_undirected(edgelist)
#' ret2 <- rewire_undirected(edgelist, e = ret1$lbound$e, iteration = 500)
#' plot(ret2$rho$Iteration, ret2$rho$rho)
#' ret3 <- joint_dist_undirected(edgelist, targetRho = 0.5)
#' ret4 <- rewire_undirected(edgelist, e = ret3$e, iteration = 500)
#' plot(ret4$rho$Iteration, ret4$rho$rho)
#' 
rewire_undirected <- function(edgelist, e, 
                              iteration = 1, nattempts = NA, 
                              rewire_track = FALSE) {
  if (is.na(nattempts)) nattempts <- nrow(edgelist)
  ...
  result <- list(rho = rho,
                 edgelist = edgelist)
  if (rewire_track) {
    colnames(ret$rewire_history) <- c("Attempt", "Edge1", "Edge2", "RewireType", "Accepted")
    ret$rewire_history[, 1:4] <- ret$rewire_history[, 1:4] + 1
    result$rewire_history <- ret$rewire_history
  }
  return(result)
}
