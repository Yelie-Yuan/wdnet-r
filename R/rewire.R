...
#' Degree preserving rewiring towards the target edge-level joint distribution.
#'
...
#'
#' @examples
#' edgelist <- edgelist <- rpanet(30000, control = panet.control(
#'    alpha = 0.3, beta = 0.1, gamma = 0.3, xi = 0.3,
#'    delta_out = 1, delta_in = 1))$edgelist
#' r <- list('out-out' = -0.1, 'out-in' = 0.5, 'in-out' = 0.4, 'in-in' = 0.4)
#' ret1 <- joint_dist(edgelist, targetRho = r)
#' ret2 <- rewire(edgelist, joint_e = ret1$joint_e, nloops = 500)
#' ret2 <- rewire(edgelist, joint_e = ret1$joint_e, nloops = 10, otherMetrics = TRUE)
rewire <- function(edgelist, joint_e, 
