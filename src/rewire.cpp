...
Rcpp::List rewire_directed_cpp(
  ...
  // arma::mat rewire_history(iteration * nattempts, 4, arma::fill::zeros);
  ...
  arma::mat rewire_history(hist_row, 4, arma::fill::zeros);
  for (int n = 0; n < iteration; n++) {
    for (int i = 0; i < nattempts; i++) {
      ...
      if (rewire_track) {
        rewire_history(count, 0) = count;
        rewire_history(count, 1) = e1;
        rewire_history(count, 2) = e2;
      }
      s1 = index_s[e1];
      s2 = index_s[e2];
      t1 = index_t[e1];
  ...
  if (rewire_track) {
    ret["rewire_history"] = rewire_history;
  }
  ret["out_out"] = out_out;
  ret["out_in"] = out_in;
  ret["in_out"] = in_out;
...
//'
// [[Rcpp::export]]
Rcpp::List rewire_undirected_cpp(
    int iteration,
    int nattempts,
  ...
  arma::mat rewire_history(hist_row, 5, arma::fill::zeros);
